...
export function TextField({ label, name, onChange, placeholder, value }: TextFieldProps) {
  return (
    <div className="form-control">
      ...
      <input id={name} data-testid={kebabCase(name)} type="text" className="input-bordered input input-sm w-full" placeholder={placeholder} value={value} onChange={event => onChange(event.target.value)} />
    </div>
  );
}
