...
describe("buildAllocationEntries", () => {
  ...
  it("returns undefined when all values are zero or undefined", () => {
    expect.hasAssertions();
    const allocation: Allocation = { us: 0, uk: undefined, europe: 0 };
    expect(buildAllocationEntries(allocation)).toBeUndefined();
  });
  ...
  it("formats keyMapping keys with human-readable labels", () => {
    expect.hasAssertions();
    const allocation: Allocation = { communicationServices: 0.3, consumerDiscretionary: 0.2, consumerStaples: 0.1, uk: 0.15, us: 0.25 };
  ...
  it("sorts entries by value descending", () => {
    expect.hasAssertions();
    const allocation: Allocation = { europe: 0.1, us: 0.6, asia: 0.3 };
    const entries = buildAllocationEntries(allocation);
    expect(entries).toBeDefined();
  ...
  it("cycles through fallbackColors for more than 11 entries", () => {
    expect.hasAssertions();
    const allocation: Allocation = {};
    for (let i = 0; i < 13; i++) allocation[`unknown${String(i)}`] = 0.05;
    const entries = buildAllocationEntries(allocation);
    expect(entries).toBeDefined();
    expect(entries!.length).toBeGreaterThanOrEqual(12);
    // Index 11 cycles back to fallbackColors[0]
    const idx11 = entries!.find(e => e.key === "unknown1");
    const idx0 = entries!.find(e => e.key === "unknown0");
    expect(idx11).toBeDefined();
    expect(idx0).toBeDefined();
  });

  it("appends Other entry when sum is below 0.95 threshold", () => {
    expect.hasAssertions();
    const allocation: Allocation = { us: 0.5, europe: 0.3 };
    const entries = buildAllocationEntries(allocation);
    expect(entries).toBeDefined();
  ...
  it("does not append Other entry when sum is exactly 0.95", () => {
    expect.hasAssertions();
    const allocation: Allocation = { us: 0.6, europe: 0.35 };
    const entries = buildAllocationEntries(allocation);
    expect(entries).toBeDefined();
    expect(entries!.find(e => e.key === "other")).toBeUndefined();
  });

  it("does not append Other entry when sum exceeds 0.95", () => {
    expect.hasAssertions();
    const allocation: Allocation = { us: 0.6, europe: 0.4 };
    const entries = buildAllocationEntries(allocation);
    expect(entries).toBeDefined();
    expect(entries!.find(e => e.key === "other")).toBeUndefined();
  });
